...
// --- 사도 데이터 확장 ---
// 각 사도에 stats(능력치)와 abilities(특수 능력) 속성 추가
const angelData = {
    adam: { 
...
const StatBar = ({ label, value }) => {
    ...
    useEffect(() => {
        gsap.fromTo(barRef.current, { width: '0%' }, { width: `${value}%`, duration: 1, delay: 0.5, ease: 'power3.out' });
    }, [value, gsap]);
    ...
    return (
        <div className="stat-item">
            ...
            <div className="stat-bar-container">
                <div className="stat-bar-fill" ref={barRef}></div>
            </div>
...
const AngelsPage = ({ onBack }) => {
    ...
    const handleSelectAngel = (angelKey) => {
        ...
        if(selectedAngel && selectedAngel.name === newAngel.name) {
            setSelectedAngel(null); // 다시 클릭하면 선택 해제
        } else {
